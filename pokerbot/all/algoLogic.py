...
class AlgoDecisions(PokerDecisionMaking):
    ...
    def get_threshold(
        ...
        if flush_draw:
            ...
            if current_bet > 0:
                ...
                if self.was_reraised:
                    ...
                    threshold = (
                        PokerHands.STRAIGHT
                    )  # eh they could also have a straight
            else:
        ...
        if (
            ...
            threshold = min(
                PokerHands.merge(PokerHands.PAIR, PokerHands.TWO_PAIR), threshold
            )
    ...
    def make_preflop_decision(
        ...
        if equity < pot_odds:
            log.info(
                f"Preflop equity less than pot odds, folding ({equity} < {pot_odds})"
            )
            return fold_or_check(facing_bet)
        else:
            log.info(
    ...
    def on_turn(
        ...
        print(
            "Our turn!",
            Card.ints_to_pretty_str(hole_cards),
        ...
        if approximate_pot:
            approximate_pot = math.floor(
                ((active_opponents + 1) * facing_bet)
                / (1.5 if facing_bet != big_blind else 1)
            )
            if total_pot < approximate_pot:
                log.info(
                    f"Assuming some callers behind, pot value ({total_pot}) "
        ...
        if (
            ...
            betting_equity = calculate_equity(
                ...
                community_cards,
                active_opponents,
                threshold_hand_strength=PokerHands.merge(
                    PokerHands.PAIR, PokerHands.TWO_PAIR
                ),
                threshold_players=1,
            )
            log.info(
                f"River, >=50BB pot, <=2 ops, calculate betting equity assuming two pair/pair 50/50 split ({betting_equity})"
            )
            ...
            if (
                ...
                log.info(
                    "C-betting"
                )  # not really a c bet more like bet more than usual if c bet criteria is met
                # cause the bot already likes betting flops
                betting_equity = min(
                    0.4, equity
                )  # the 0.4 is bc of infinity vertical asymptote at 0.5
