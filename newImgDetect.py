...
class PokerImgDetect:
    ...
    def prepare_ss(screenshot: bytes):
        ...
        return cv2.imdecode(fullimg, cv2.IMREAD_COLOR)

    @staticmethod
    def magic_detection(fullimg: np.ndarray, wanted: np.ndarray):
        w, h = wanted.shape[:-1]
    ...
    def load_images(self):
        self.SIT_BUTTON_BYTES = cv2.imread(self.imgs_path + "sit.png")

    def detect_sit_button(self, screenshot: cv2.typing.MatLike):
        return self.magic_detection(screenshot, self.SIT_BUTTON_BYTES)
...
if __name__ == "__main__":
    ...
    img = np.frombuffer(org_img, np.uint8)
    img = cv2.imdecode(img, cv2.IMREAD_COLOR)
    locations = poker_img_detect.detect_sit_button(org_img)
